...
FROM (SELECT company, AVG(star_rating)								--answer: 41 (40 if you don't count the null value)			
FROM data_analyst_jobs
...
FROM data_analyst_jobs												--answer: 754 unique job titles (774 with iLIKE function to make case insensetive)
WHERE title LIKE '%Analyst%';										--1636 if not unique (1669 after accounting for case using iLIKE)
...
AND title NOT LIKE '%Analytics%';									--indicating the SQL search is case sensitive, when using iLIKE - titles all include Tableau and Visualization
...
ORDER BY counts DESC;
			...
			--CLASS NOTES: 
			--how to select the nth value in a database: 
